...
public class ConsumeMessageConcurrentlyService implements ConsumeMessageService {
    ...
    public void submitConsumeRequest(
        ...
        final int consumeBatchSize = this.defaultMQPushConsumer.getConsumeMessageBatchMaxSize(); // 默认为32条
    ...
    public void processConsumeResult(
        ...
        switch (status) {
            case CONSUME_SUCCESS:
                // ackIndex 默认 Integer.MAX_VALUE
                if (ackIndex >= consumeRequest.getMsgs().size()) {
                    ackIndex = consumeRequest.getMsgs().size() - 1; // ackIndex不能超过消息列表的大小
                }
                int ok = ackIndex + 1;
    ...
    public boolean sendMessageBack(final MessageExt msg, final ConsumeConcurrentlyContext context) {
        // 给到broker的延迟级别为 0，具体该使用什么延迟等级是在 Broker 收到该 Message 之后决定
    ...
    class ConsumeRequest implements Runnable {
        ...
        public void run() {
            ...
            if (null == status) { // 如果消费结果为 null，那么就认为消费失败
                if (hasException) {
            ...
            if (null == status) { // 如果消费结果为 null，那么就认为消费失败
                log.warn("consumeMessage return null, Group: {} Msgs: {} MQ: {}",
                    ...
                    msgs,
                    messageQueue);
                status = ConsumeConcurrentlyStatus.RECONSUME_LATER; // 设置为稍后消费
            }
            ...
            if (!processQueue.isDropped()) {
                // Consumer 此时会调用 processConsumeResult 来根据 status 进行对应的处理
