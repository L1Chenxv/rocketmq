...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST: // 实例化之后默认状态
                ...
                // 注册到 MQClientInstance 的 producerTable 中
    ...
    private SendResult sendDefaultImpl(
        ...
        this.makeSureStateOK();
        // msg validation: topic and body
        Validators.checkMessage(msg, this.defaultMQProducer);

        final long invokeID = random.nextLong();

        // beginTimestampFirst: the first time to send message
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 默认是同步发送，重试次数为 2 次 总投递次数为 3 次（投递 1 次， 重试 2 次）
            // 异步则总投递次数为 1 次
                        ...
                        switch (communicationMode) {
                            ...
                            case SYNC:
                                if (sendResult.getSendStatus() != SendStatus.SEND_OK) {
                                    // 当发送失败时，如果开启了重试另外一个 Broker 机制，则重试发送
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            // topic 相关数据不存在或 MessageQueue 列表为空，创建一个并放入缓存，更新路由信息
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        SendResult sendResult = null;
        // 标记消息为 Half Message  MessageConst.PROPERTY_TRANSACTION_PREPARED = true
            ...
            case SEND_OK: {
                try {
                    ...
                    if (null != localTransactionExecuter) {
                        ...
                        // 如果设置了 transactionListener, 这里会通过 listener 来执行本地事务
        ...
        try {
            // 给事务消息的流程收尾 -> 向 Broker 发送一个 RequestCode 为 END_TRANSACTION 的请求
